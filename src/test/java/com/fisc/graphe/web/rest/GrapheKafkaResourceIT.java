package com.fisc.graphe.web.rest;

import com.fisc.graphe.config.KafkaProperties;
...
class GrapheKafkaResourceIT {
    ...
    void setup() {
        ...
        GrapheKafkaResource kafkaResource = new GrapheKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/graphe-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/graphe-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
